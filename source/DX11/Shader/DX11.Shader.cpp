...
HRESULT DX11_Shader::LoadShadersFromFile_withGS(
    const std::wstring& filePath,
    const char* vsEntry, const char* gsEntry, const char* psEntry,
    const char* vsTarget, const char* gsTarget, const char* psTarget
) {
    HRESULT hr = S_OK;
    ...
    if (gsEntry)
    {
        ...
        hr = CompileShaderFromFile(filePath, gsEntry, gsTarget, gsBlob);
        if (FAILED(hr)) return hr;
        hr = framework::directx::GetDevice()->CreateGeometryShader(
...
void DX11_Shader::Bind()
{
    ...
    framework::directx::GetContext()->GSSetShader(geometryShader.Get(), nullptr, 0);
