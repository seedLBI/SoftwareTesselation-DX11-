...
public:
	...
	void BindConstantBufferToVS(const std::string& name);
	void BindConstantBufferToPS(const std::string& name);
...
private:
	...
	struct CBufferBinding {
		...
		UINT bindSlot = UINT_MAX;
		UINT size = 0;
	};

	std::unordered_map<std::string, CBufferBinding> cbuffers;
